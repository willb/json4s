...
import org.json4s.macro_readers.{JsonObjectReader, JsonArrayIterator}
...
class BsonObjectReader(obj: BasicBSONObject) extends macro_readers.JsonObjectReader {
  ...
  // Option forms
  def optObjectReader(key: String): Option[JsonObjectReader] = obj.get(key) match {
    case obj: BasicBSONObject => Some(new BsonObjectReader(obj))
    case _ => None
  }

  def optArrayReader(key: String): Option[JsonArrayIterator] = obj.get(key) match {
...
class BsonIterator(lst: BasicBSONList) extends macro_readers.JsonArrayIterator {

  private val it = lst.iterator()

  private def next = if (it.hasNext) it.next() else null

  def getNextObjectReader: Option[JsonObjectReader] = next match {
    case obj: BasicBSONObject => Some(new BsonObjectReader(obj))
    case _ => None
  }

  def getNextArrayReader: Option[JsonArrayIterator] = next match {
