...
private[json4s] class JsonStringCursor(txt: String) extends JsonTextCursor { self =>
  ...
  override def failParse(msg: String) = throw new ParserUtil.ParseException(msg + "Remainder: " + remainder, null)
}