...
object JsonParserSpec extends Specification with JValueGen with ScalaCheck {
  import jackson.JsonParser.{ parse, parseOpt }
  import jackson.JsonMethods.{render, pretty, compact}


  "A JSON Parser" should {
    "Any valid json can be parsed" in {
      val parsing = (json: JValue) => { parse(pretty(render(json))); true must beTrue }
      prop(parsing)
    }
