...
class MapValueReaderSpec extends Specification {
  ...
  "A MapValueReader with a DotSeparator" should {
    ...
    "get my stuff right" in {
      //val expected = ThingWithJunk("Bob",Junk(2,"SomeJunk..."))
	    val data = Map("dog.name"->"Bob","dog.junk.in1"->2,
					  "dog.junk.in2"->"SomeJunk...")
      val rdr = new MapValueReader(data)
      rdr.forPrefix("dog").forPrefix("junk").values must_== Map("in1"->2,"in2"->"SomeJunk...")
    }
